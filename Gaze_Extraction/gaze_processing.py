...
def main(args):
    ...
    csv_outfile = f'../results/{fn.replace(suffix, "_gaze360_out.csv")}'

    avg_csv_outfile = f'../results/{fn.replace(suffix, "_avg_gaze360_out.csv")}'
    ...
    mpii_df = pd.read_csv(f'../results/{gaze_mpii}_gaze_output.csv', names=nm)
