...
def estimateHeadPose(landmarks):
    ...
    ret, rvec, tvec = cv2.solvePnP(face_model, landmarks, cam_matrix, np.zeros((5,1)), flags=0)
...
df = pd.read_csv(f'/home/vislab/RecordingSetup/results/{fn}_gaze_output.csv', names=nm)
...
for n, frame in tqdm(enumerate(reader)): 
    if n < 4:
        ...
        if i not in ind_frames:
            ...
            image = cv2.rectangle(image, (bbox[0]-30,bbox[1]-30), (bbox[2]+30,bbox[3]+30), (255,255,255))
