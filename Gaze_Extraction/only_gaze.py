...
def annotateFrame(image, mpii_gaze, gaze360_gaze, avg_gaze, f_center, origin, bbox):
    image = drawGaze(image,gaze360_gaze,f_center,(255,255,255))
    image = drawGaze(image, mpii_gaze, origin, (255,0,0))
    image = drawGaze(image,avg_gaze,f_center,(0,255,0))
...
avg_csv_outfile = f'/home/vislab/RecordingSetup/results/{fn.replace(suffix, "_avg_gaze360_out.csv")}'
...
mpii_df = pd.read_csv(f'/home/vislab/RecordingSetup/results/{gaze_mpii}_gaze_output.csv', names=nm)
...
for n, frame in tqdm(enumerate(reader)): 
    if n < 4:
        ...
        if i not in ind_frames:  # If face was not detected in frame
            ...
            image = annotateFrame(image,mpiigaze,gaze, avg_gaze, face_center, origin, bbox)
            out.append_data(image)
out.close()
