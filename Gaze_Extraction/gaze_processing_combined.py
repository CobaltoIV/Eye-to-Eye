    ...
    for n, frame in tqdm(enumerate(reader)): 
        if n < 4:
            ...
            if i not in ind_frames:  # If face was not detected in frame
                ...
                mpii_gaze2d = Gaze3DTo2D(mpiigaze,face_center,ext_rmat,ext_tvec)
                mpii_gaze2d = convertToScreenCoordinates(mpii_gaze2d, mon_w, mon_h)
                
                gaze2d = Gaze3DTo2D(gaze,face_center,ext_rmat,ext_tvec)
                gaze2d = convertToScreenCoordinates(gaze2d, mon_w, mon_h)
                
                res.append([i, 1, box[4], face_center[0], face_center[1], face_center[2], gaze2d[0][0], gaze2d[1][0],gaze[0], gaze[1], gaze[2], mpii_gaze2d[0][0], mpii_gaze2d[1][0]])
