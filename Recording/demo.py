...
class RecordingSetup:
    ...
    def __init__(self, root):
        ...
        id_entry.grid(column=1, row=1, sticky=(W, E))
        ttk.Label(self.mainframe, text="Doctor Id").grid(column=0, row=1)

        # Error Label for Button
        self.error = StringVar()
        ttk.Label(self.mainframe, textvariable=self.error).grid(column=2, row=1)

        self.spec = StringVar()
        spec_entry = ttk.Combobox(self.mainframe, textvariable=self.spec)
        spec_entry.grid(column=1, row=2, sticky=(W, E))
        spec_entry['values'] = ('Neurologia', 'MedInt', 'MedGeral', 'Genecologia', 'Endocrinologia')
        spec_entry.state(["readonly"])
        ttk.Label(self.mainframe, text="Doctor Specialty").grid(column=0, row=2)

        
        
        self.mode =  StringVar()
        mode_entry_presential = ttk.Radiobutton(self.mainframe, text="Presential", variable=self.mode, value="Presential")
        mode_entry_presential.grid(column=1)
        mode_entry_virtual = ttk.Radiobutton(self.mainframe, text="Virtual", variable=self.mode, value="Virtual")
        mode_entry_virtual.grid(column=1)
        ttk.Label(self.mainframe, text="Type Of Consultation").grid(column=0, row=3)

        #Record Button
        self.btn = ttk.Button(self.mainframe, text="Record", command=self.record)
        self.btn.grid(
            column=1,sticky=(W, E, N, S))
        ...
        self.recNotice.grid(column=0, row=5)
    ...
    def record(self, *args):
        ...
        """
        self.p = subprocess.Popen(["ffmpeg", "-f", "video4linux2", "-s","hd720","-input_format","mjpeg", "-i", "/dev/video0", "-r", "15","-preset","veryfast", video_location])
        """
